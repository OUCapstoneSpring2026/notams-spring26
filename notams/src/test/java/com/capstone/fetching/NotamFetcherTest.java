...
class NotamFetcherTest
{
	...
	void testFetchByIcao_validCode() throws IOException, InterruptedException
	{
		...
		when( mockHttpClient.send( any( HttpRequest.class ), any(
				HttpResponse.BodyHandler.class ) ) ).thenReturn( mockResponse );
		...
		assertEquals( URI.create( BASE_URL + "?responseFormat=geoJson"
				+ "&icaoLocation=KOKC" + "&pageSize=50" + "&pageNum=2" ),
				request.uri() );

		assertEquals( "GET", request.method() );
		assertEquals( CLIENT_ID, request.headers().firstValue( "client_id" )
				.orElseThrow() );
		assertEquals( CLIENT_SECRET, request.headers().firstValue(
				"client_secret" ).orElseThrow() );
		assertEquals( "application/json", request.headers().firstValue(
				"Accept" ).orElseThrow() );
	...
	{
		when( mockHttpClient.send( any( HttpRequest.class ), any(
				HttpResponse.BodyHandler.class ) ) ).thenThrow( new IOException(
						"Connection refused" ) );
	...
	@Test
	void testFetchByIcao_paginationBoundaries() throws IOException,
												...
												InterruptedException
	{
		...
		String result = fetcher.fetchByCoordinates( 35.393, -97.600, 50, 25,
				3 );
	...
	{
		when( mockHttpClient.send( any( HttpRequest.class ), any(
				HttpResponse.BodyHandler.class ) ) ).thenThrow(
	...
	@Test
	void testFetchByCoordinates_invalidPagination()
	{
		assertThrows( IllegalArgumentException.class, () -> fetcher
				.fetchByCoordinates( 35.0, -97.0, 50, 0, 1 ) );

		assertThrows( IllegalArgumentException.class, () -> fetcher
				.fetchByCoordinates( 35.0, -97.0, 50, 1001, 1 ) );

		assertThrows( IllegalArgumentException.class, () -> fetcher
				.fetchByCoordinates( 35.0, -97.0, 50, 100, 0 ) );

		assertThrows( IllegalArgumentException.class, () -> fetcher
				.fetchByCoordinates( 35.0, -97.0, 50, 100, -1 ) );
	}
